...
//触发jump回调的方式有俩种：
//1.点击页码的时候 会触发jump回调
//2.调用了laypage.render()方法 就会触发jump回调


        jump:function(obj,first){
          //可以通过first的值来判断通过那种方式来触发jump回调
          ...
          //把最新的条目数 赋值到q 这个查询对象的 pagesize 属性中
          q.pagesize=obj.limit
          //根据最新的q获取对应的数据列表并且渲染表格
          // initTable()
 ...
 layer.msg('删除文章成功！')
 //当数据删除完成后，需要判断当前这页还有剩余的数据
 //如果没有剩余的数据了，则让页码-1
 //再重新调用 initTable() 方法
   if(len===1) {
     //len的值等于1 证明删除完毕之后页面就没有数据了
     //页码值最小必须是1
